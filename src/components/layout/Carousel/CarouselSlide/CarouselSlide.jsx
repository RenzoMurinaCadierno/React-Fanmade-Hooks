...
export default function CarouselSlide({
  ...
  const { transitionDirection, activeName, ptd } = useContext(Carousel.context)
  ...
  const [mountRevCN, triggerMountRevCN] = useClassNameToggle({
    className:
      classNames.animateMountRev ??
      classes[
        "animate-mount-" + (transitionDirection === "left" ? "right" : "left")
      ],
    timeout
  })

  const [unmountRevCN, triggerUnmountRevCN] = useClassNameToggle({
    className:
      classNames.animateUnmountRev ??
      classes[
        "animate-unmount-" + (transitionDirection === "left" ? "right" : "left")
      ],
    timeout
  })
  ...
  useEffect(() => {
    ...
    console.log(transitionDirection, ptd)
    if (!isActive && isMounted) {
      // trigger unmounting animation
      transitionDirection === ptd ? triggerUnmountCN() : triggerUnmountRevCN()
      ...
      transitionDirection === ptd ? triggerMountCN() : triggerMountRevCN()
      // triggerMountCN()
      setActiveSt(true)
    }
